...
Point direction(double ta, double tb, double tc){
    ...
    temp.xAngle = acos(x/sqrt(pow(x,2)+pow(y,2)))*(180/PI);
    temp.yAngle = acos(sqrt(pow(x,2)+pow(y,2))/kDist)*(180/PI)+90;
    //test
    //printf("%f\t %f\t %f\n", x, y, z);
    return Point {x, y, z};
}
...
angleSet direction_angle(double ta, double tb, double tc){
    ...
    temp.xAngle = acos((x - xOffset)/sqrt(pow(x - xOffset,2)+pow(y - yOffset,2)))*(180/PI);
    if (z < 0) {
        temp.yAngle = -acos(sqrt(pow(x - xOffset ,2)+pow(y - yOffset,2))/kDist)*(180/PI)+90;
    } else {
        temp.yAngle = acos(sqrt(pow(x - xOffset,2)+pow(y - yOffset,2))/kDist)*(180/PI)+90;
    }
...
// int main() {
//     angleSet angle = direction(-200e-6, -100e-6, -50e-6);
